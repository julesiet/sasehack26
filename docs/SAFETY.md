...
`POST /composio/execute` may create a Gmail draft (`GMAIL_CREATE_EMAIL_DRAFT`) or send (`GMAIL_SEND_EMAIL`) when the HTTP caller asks. Default remains `GMAIL_GET_PROFILE`. A `notify_caretaker` preview never calls Gmail; only a human-approved send invokes `GMAIL_SEND_EMAIL` to `juleselvandrade@gmail.com` with `familyEmailCopy` (a Jules-addressed HTML report from Maria's seed; worth-reviewing language, never a diagnosis), with a mock fallback when Composio is unconfigured or needs authorization so CI stays green. Display names may be James Alvarez, Jules, Sarah, or Emily; delivery stays Jules's Gmail. A model actor cannot self-approve a send. `pnpm playground` with a spoken family email must stop at `pendingApproval.tool` `notify_caretaker` and must not auto-approve send.
