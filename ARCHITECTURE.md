...
`apps/mobile/src/screens/CaretakerScreen.tsx`. Family view of Maria's same `DEFAULT_SESSION_ID`. The `default` session is created with `getMariaDemoSession()` so Overview already has appointment, wheelchair Uber, consent, and the three-turn ride thread. Polls `GET /sessions/:sessionId` every 2s so a later senior booking replaces that seed. Projection is `buildCaretakerDashboard()` in `packages/shared/src/caretaker-dashboard.ts`. Layout matches the designed iPhone: date + “Hello, Margaret”, care notes (worth reviewing, 15-minute lead, “No diagnosis noted”), contacts, then an orange Overview bowl with appointment, selected ride, consent / approval record, and activity summary. Badge is `CONFIRMED` / `WAITING` / `DECLINED`. Care notes never diagnose. Invites are not sent. Other session ids stay empty for tests and the playground.
