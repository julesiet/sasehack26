...
`apps/mobile/src/screens/SeniorScreen.tsx`. **Home** is the sun greeting. Speaking or typing there starts a new Chat history row and makes it the current thread. **Chat** opens the history list (`ChatHistoryScreen`): newest thread first, titles are short topic labels (**Doctor ride**, **Medication reminder**, **Hospital visit**, **Family update**). Tapping a row opens that thread in `SeniorChatScreen`. **New chat** on the list also starts an empty thread on the same session (`POST /conversation/chats`). Tapping Home always returns to the welcome. The confirmation card only appears while a high-risk action is waiting (or just after Confirm / Cancel) on the live thread.
...
`POST /sessions/:sessionId/reset` body `{ preset?: "seed" | "live-demo" }` replaces that session. `seed` is `getMariaDemoSession()` on `default` (already-booked WAV). `live-demo` is `getMariaLiveDemoSession()`: appointment, hospital + medication chats, Tasks, care signal, **no** ride booking. Use it before the 3-minute script (`#12`) so caretaker Overview starts idle and Maria can book on stage. Spoken lines live in `packages/shared/src/demo-script.ts`; print with `pnpm demo:pdf`. `KASAMA_DEMO=1` forces the rules-based turn even if `MODEL_API_KEY` is set.

`POST /conversation/chats` body `{ sessionId?, chatId? }` starts a new thread or selects an existing one. Speaking or typing on Senior Home, or **New chat** on the list, starts a thread. Topics do not split on their own inside an open thread.
...
When `MODEL_API_KEY` is set, `apps/api/src/harness.ts` asks ChatGPT (Chat Completions, default `gpt-4o-mini`, optional `MODEL_NAME`) to propose Kasama tools. Each call runs through `invokeTool` as `actor: "model"`. If the key is missing, OpenAI throws, or `KASAMA_DEMO=1`, `apps/api/src/conversation.ts` uses the original rules-based turn. Composio is not on this loop.
...
- Session memory: `sessionView.conversation` = `{ chats, activeChatId, turns, activeRequest, clarificationsAsked, plan, failure }`. `turns` is the active chat. History lists `chats` newest-first with `chatTitleForIntent` labels. Maria can say "yes" on the next turn without restating the appointment. A spoken clock (`today at 3 pm`) is stored on `activeRequest.arriveBy` and used for Uber search, Kasama's copy, and the confirmation card — do not say tomorrow unless she asked for that day or the pickup is tomorrow. If she does not name a time, Kasama falls back to 15 minutes before the appointment (or 30 minutes from now for a free-form destination).
- After `find_ride_options`, ChatGPT must not list UberX / WAV or say it found two Uber options. The iPhone shows those as tappable cards. A listing reply is rewritten to the appointment/pickup line and is not stored as a Kasama bubble.
...
- `packages/shared/src/tools.ts` — tool names, inputs, results (Uber products: `UberX`, `WAV`; local `save_medication_reminder` / `save_hospital_visit`). `notify_caretaker` inputs and draft results carry optional recipient identity fields. Hospital `timeLabel` is speakable (`Thursday at 10:00 AM`); ISO stamps from ChatGPT are formatted before the card. Medication `frequency` is speakable (`Every 4 days`); a bare `every` is filled in from `intervalDays` before Tasks.
- `packages/shared/src/time-label.ts` — ISO → weekday + clock for hospital times; `formatPickupWhen` for ride cards (`Today at 3:00 PM`)
...
- `packages/shared/src/conversation.ts` — voice loop contracts: turn request/response, chats (`conversationChatSchema`, topic titles, `startNewChat`), `activeRequest` (including `arriveBy` when Maria names a clock), `plan`, `failure`, `pendingApproval`, `MAX_CLARIFICATIONS_PER_REQUEST`, `MAX_TOOL_ROUNDS_PER_TURN`, transcribe response
...
- `packages/shared/src/demo-session.ts` — iOS `default` session start state: appointment, UberX + WAV options, seeded WAV booking (`UBER-WAV-SEED`), senior approval, Lisinopril reminder, St. Mary's visit, Tasks items, and `conversation.chats` (hospital visit, medication reminder, doctor ride). Senior Chat history, caretaker activity, and Tasks all read this from `GET /sessions/default` — do not add a second transcript store. `getMariaLiveDemoSession()` is the unbooked variant for the 3-minute demo.
- `packages/shared/src/demo-script.ts` — printed 3-minute demo lines (`#12`). Tests, `pnpm demo:rehearse`, and `pnpm demo:pdf` read this same object.
