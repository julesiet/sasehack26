...
- `packages/shared/src/demo-session.ts` — iOS `default` session start state: appointment, UberX + WAV options, seeded WAV booking (`UBER-WAV-SEED`), senior approval, and `conversation.turns`. Senior Chat, caretaker activity, and a future chat-history page must all read this from `GET /sessions/default` — do not add a second transcript store.
