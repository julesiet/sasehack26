...
- `GET /sessions/:sessionId` — in-memory session view (current request, pending approval, last approval, last Uber options, appointment, last Uber booking, last medication reminder, last hospital visit, tasks, caretaker activity, care signal, session events). The iOS `default` session starts with `getMariaDemoSession()` (Maria's ride thread + seeded WAV). Other ids start empty.
...
Caretaker dashboard (`#9`): Home **Dev** → **Caretaker**. Same `default` session as Maria, polled every 2s. That session is prefilled with Maria's ride thread so Overview and senior Chat start with the same appointment, wheelchair Uber, consent, and three turns. Designed screen shows care notes (never a diagnosis), contacts, appointment, selected Uber, consent / approval record, and a short activity summary. **Maria** switches to Senior mode on the same phone. A later chat-history page ([#45](https://github.com/mawerb/sasehack26/issues/45)) must read `session.conversation.turns` — do not invent a second store. See [ARCHITECTURE.md](ARCHITECTURE.md#caretaker-dashboard-9).
