...
- `GET /sessions/:sessionId` — in-memory session view (current request, pending approval, last approval, last Uber options, appointment, last Uber booking, last medication reminder, last hospital visit, tasks, caretaker activity, care signal, session events, `conversation.chats`). The iOS `default` session starts with `getMariaDemoSession()` (three titled chats — hospital visit, medication reminder, doctor ride — plus seeded WAV). Other ids start empty.
