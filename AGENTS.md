...
- `POST /conversation/turn` — `{ transcript, sessionId?, chatId? }` → Kasama's reply + `kind` + `plan` + `failure` + `pendingApproval`. Optional `chatId` selects that thread first. ChatGPT plans when `MODEL_API_KEY` is set unless `KASAMA_DEMO=1` (rules-based copy for the live demo). Every tool still goes through policy + audit. ChatGPT is never allowed to book or send. A spoken clock time (`today at 3 pm`) is the pickup/arrive-by, not Maria's seeded tomorrow appointment. If she does not name a time, Kasama falls back to 15 minutes before the appointment (or 30 minutes from now for a free-form destination). A spoken yes after a ride plan opens the $24.50 checkpoint; a second yes (or tap) books as `senior`.
...
- `POST /speech/speak` — `{ text }` → MPEG audio of Kasama (same key, plus Text to Speech on the key; `501` otherwise, device falls back to iOS speech). Speed/rate follow Maria's `speaksSlowly` preference. Ride and family-email checkpoints are spoken twice when `repeatsConfirmations` is true; Chat stays the single prompt.
...
- `POST /composio/execute` — `{ userId?, toolSlug?, arguments? }` → session tool result + `logId`. Defaults to `GMAIL_GET_PROFILE`. Pass `GMAIL_CREATE_EMAIL_DRAFT` for a caretaker draft or `GMAIL_SEND_EMAIL` to send to `juleselvandrade@gmail.com`. A successful draft includes `data.draft_id` even when Composio only returns `id`. `409` with a Connect Link if Gmail is not connected. `GMAIL_SEND_DRAFT` is rejected. Human-approved `notify_caretaker` sends through this Composio path; previews never call Gmail, and an unconfigured or unauthorized provider falls back to the CI-safe mock.

Omitted `sessionId` on a tool call is stored as `default`. Senior and caretaker clients poll the same `sessionId`.

Tools: `get_appointment`, `find_ride_options`, `book_ride`, `notify_caretaker`, `save_medication_reminder`, `save_hospital_visit`. After a human yes, `book_ride` returns `status: "booked"` and a confirmation id, or `success: false` if the confirmation cannot be proven. `notify_caretaker` drafts automatically (`preview`, not sent). Send needs a human `approvalToken` (actor ≠ `model`) and uses Composio Gmail for `juleselvandrade@gmail.com`, with a mock fallback when Composio is unconfigured or needs authorization. The Gmail body is an HTML family report for Jules (`is_html: true`) built from Maria's seeded care data, with headers and tables for the spoken summary, appointment, tasks, wearable facts, and worth-reviewing notes. Delivery stays `juleselvandrade@gmail.com`. A failed send keeps the checkpoint pending for retry and does not treat `lastApproval` as proof the email went out. `save_medication_reminder` is a Tasks item, not a prescription change: “set a reminder” / “remind me” gathers the name and how often, then the first human yes tries a stub health sync (fails, recoverable error), and Confirm on that error saves locally. Tasks shows a speakable cadence (`Every 4 days`), never a bare `every`. `save_hospital_visit` saves St. Mary's appointment details after a human yes — “schedule an appointment” starts that thread. Scheduled time is speakable (`Thursday at 10:00 AM`), never a raw ISO stamp. Not live EHR.
...
Senior screen design tokens live in `apps/mobile/src/theme.ts`. Conversation phases and what each looks like are in [ARCHITECTURE.md](ARCHITECTURE.md#senior-conversation-screen-4). Senior tabs: Home (sun welcome; talking here starts a new Chat history row and makes it current), Chat (history list, current thread first; tap a topic label to open that thread), Tasks (confirmed medication reminders and saved hospital visits). Chat is available once the session has chats (the iOS `default` session is seeded). Active tab is sun orange. On Home the compact tab bar is 80% opaque over the sun. After a ride search, the open ride thread shows two Uber option cards (UberX and wheelchair WAV) as the selection — Kasama does not add a chat bubble listing those options or prices. Tapping a card selects that product through the conversation turn (same as saying "the wheelchair Uber" / "the UberX"). Confirmation is a descriptive Cancel / Confirm card for the selected option. Finding and booking show a current-action card so the screen never looks idle. Medication reminder, health-sync error, and hospital appointment cards follow the same large-type Cancel / Confirm pattern (`#41`). A family email preview is a **MESSAGE TO** card (recipient, relationship, exact message, urgency, whether health information is included, Preview only, 68pt Cancel / Confirm) — it is not the ride ConfirmationCard or the Uber option rows. Voice yes/no and tap Confirm both send. After Confirm the card reads Sent with the same facts; Cancel reads Not sent. If Gmail fails, Confirm stays for retry and the card does not read Sent. The saved hospital card is only the just-finished confirmation; the next question leaves Chat as bubbles and keeps the visit on Tasks. Tap targets ≥ 68pt on senior actions.

Caretaker dashboard (`#9`): Home **Dev** → **Caretaker**. Same `default` session as Maria, polled every 2s. That session is prefilled with Maria's three titled chats (hospital visit, Lisinopril reminder, doctor ride) so Overview, Tasks, and Chat history start from the same store. Designed screen shows care notes (never a diagnosis), contacts (James Alvarez, Jules, Sarah, Emily), appointment, selected Uber, consent / approval record, a **FAMILY UPDATE** card when a family email is drafted / sent / not sent (recipient, relationship, exact message, urgency, health sharing, Preview only while still a draft), and a short activity summary. Jules is a family contact; every delivered email still goes to `juleselvandrade@gmail.com`. Tapping **Care notes** opens the care-aware view (`#10`); **Notify daughter** there uses the same `notify_caretaker` tool (display recipient Sarah; delivery is Jules's Gmail) and Confirm sends as `actor: "caretaker"`. Cancel, the backdrop, and Android back decline that draft the same way. The orange arrow returns to this dashboard. **Maria** switches to Senior mode on the same phone. Chat history ([#45](https://github.com/mawerb/sasehack26/issues/45)) reads `session.conversation.chats` / `turns` — do not invent a second store. See [ARCHITECTURE.md](ARCHITECTURE.md#caretaker-dashboard-9).
