...
Caretaker dashboard (`#9`): Home **Dev** → **Caretaker**. Same `default` session as Maria, polled every 2s. That session is prefilled with Maria's three titled chats (hospital visit, Lisinopril reminder, doctor ride) so Overview, Tasks, and Chat history start from the same store. Designed screen shows care notes (never a diagnosis), contacts, appointment, selected Uber, consent / approval record, and a short activity summary. Tapping **Care notes** opens the care-aware view (`#10`); **Notify daughter** there drafts `notify_caretaker` (display recipient Sarah; delivery is Jules's Gmail) and Confirm sends as `actor: "caretaker"`. The orange arrow returns to this dashboard. **Maria** switches to Senior mode on the same phone. Chat history ([#45](https://github.com/mawerb/sasehack26/issues/45)) reads `session.conversation.chats` / `turns` — do not invent a second store. See [ARCHITECTURE.md](ARCHITECTURE.md#caretaker-dashboard-9).
