...
Speech debug (`#17`) is behind Home **Dev** → **Speech debug**. Placeholder layout, system buttons, large default text. Same voice loop as Senior, session id `speech-debug` (not `default`). Mic denial is visible and recoverable. Delete or keep hidden when the designed screen is enough.
