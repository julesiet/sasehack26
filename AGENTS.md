...
- `GET /sessions/:sessionId` — in-memory session view (current request, pending approval, last approval, last Uber options, appointment, last Uber booking, last medication reminder, last hospital visit, tasks, caretaker activity, caretaker narrative, care signal, session events). The iOS `default` session starts with `getMariaDemoSession()` (Maria's ride thread + seeded WAV). Other ids start empty.
