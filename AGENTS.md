...
- `POST /composio/execute` — `{ userId?, toolSlug?, arguments? }` → session tool result + `logId`. Defaults to `GMAIL_GET_PROFILE`. Pass `toolSlug: "GMAIL_CREATE_EMAIL_DRAFT"` for a caretaker Gmail draft (not a send). `409` with a Connect Link if Gmail is not connected. Send slugs are rejected. Not wired into `notify_caretaker` or conversation (`#16`)
