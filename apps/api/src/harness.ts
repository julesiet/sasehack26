...
function systemPrompt(now: Date, state: ConversationState): string {
  ...
  return [
    ...
    "Do not call book_ride, notify_caretaker, save_medication_reminder, or save_hospital_visit. Propose the action and wait.",
    "A medication reminder only adds a task. Never change a prescription or diagnose. Hospital scheduling saves local appointment details — not a live health record.",
